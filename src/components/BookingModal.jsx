...
export default function BookingModal({ isOpen, onClose, preselectedService }) {
  ...
  // Pre-fill name from Google profile when available
  const googleFirstName = profile?.name?.split(' ')[0] || ''
  const googleLastName  = profile?.name?.split(' ').slice(1).join(' ') || ''
  const [clientName, setClientName]         = useState(googleFirstName)
  const [clientSurname, setClientSurname]   = useState(googleLastName)
  ...
  // ── Gate: exige login com Google ──
  if (!profile) {
    return (
      <ModalWrapper onClose={onClose}>
        <div className="flex flex-col items-center text-center py-8 px-2 gap-6">
          <img src={LOGO} alt="R9" className="h-16 w-auto object-contain" />
          <div>
            <h3 className="text-xl font-black mb-2">Entre para agendar</h3>
            <p className="text-text-muted text-sm max-w-xs">
              Faça login com sua conta Google para garantir seu horário com segurança.
            </p>
          </div>
          <button
            onClick={signInWithGoogle}
            className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl font-semibold text-sm border border-white/15 bg-white/5 hover:bg-white/10 transition-all"
          >
  ...
  const handleConfirm = async () => {
    ...
    // B — 1 agendamento ativo por usuário autenticado (mais seguro que por telefone)
    const hasActiveByUser = profile?.id ? await checkUserHasActiveBooking(profile.id) : false
    if (hasActiveByUser) {
      toast.error('Você já possui um agendamento ativo. Cancele o anterior para agendar novamente.')
      return
    }
    // Fallback: verificação por telefone para agendamentos sem userId
